...
class DictionaryEditor:
    ...
    def create_widgets(self):
        ...
        tk.Entry(frame, textvariable=tk.StringVar(value=self.file_path), state='readonly', width=50).grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        ...
        tk.Button(button_frame, text="Save", command=self.save_dictionary).grid(row=0, column=2, padx=5, pady=5)
    ...
    def load_dictionary(self):
        with open(self.file_path, 'r', encoding='utf-8') as f:
            ...
            self.data = pd.DataFrame(list(data.items()), columns=["Original", "Translation"])
    ...
    def update_treeview(self):
        ...
        for i, (_, row) in enumerate(self.data.iterrows()):
            ...
            self.tree.insert("", "end", iid=i, values=(row["Original"], row["Translation"]), tags=tags)
    ...
    def add_entry(self):
        ...
        if original and translation:
            new_row = pd.DataFrame({"Original": [original], "Translation": [translation]})
    ...
    def save_dictionary(self):
        data_dict = {row["Original"]: row["Translation"] for _, row in self.data.iterrows()}
