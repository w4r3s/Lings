...
class TextEditor:
    ...
    def create_widgets(self):
        ...
        tk.Entry(frame, textvariable=self.file_path, width=50).grid(
            row=0, column=1, padx=5, pady=5, sticky="ew"
        )
        tk.Button(frame, text="Load", command=self.load_file).grid(
        ...
        tk.Button(button_frame, text="Save", command=self.save_file).grid(
            row=0, column=2, padx=5, pady=5
        )
        tk.Button(button_frame, text="Save As", command=self.save_file_as).grid(
            row=0, column=3, padx=5, pady=5
        )
        tk.Button(
            button_frame,
            text="Translate Selected with DeepL",
        ...
        self.context_menu.add_command(
            label="Translate Selected (DeepL)", command=self.translate_selected
        )
        # 这里可以添加更多插件的选项
    ...
    def update_treeview(self):
        ...
        for i, (_, row) in enumerate(self.data.iterrows()):
            ...
            self.tree.insert(
                ...
                "end",
                iid=i,
                values=(row["Original"], row.get("Translation", "")),
                tags=tags,
            )
    ...
    def add_entry(self):
        ...
        if original and translation:
            self.data = pd.concat(
                [
                    self.data,
                    pd.DataFrame([{"Original": original, "Translation": translation}]),
                ],
                ignore_index=True,
            )
            self.update_treeview()
    ...
    def prompt_and_save(self, file_path):
        save_original = messagebox.askyesno(
            "Save Original", "Do you want to save the original text as well?"
        )
    ...
    def save_word_file(self, file_path, save_original):
        ...
        for _, row in self.data.iterrows():
            ...
            if original:
                p = doc.add_paragraph()
                if save_original:
                    p.add_run(original).font.color.rgb = RGBColor(
                        0, 0, 255
                    )  # 原文设置为蓝色
                    if translation:
                        p.add_run("\n" + translation).font.color.rgb = RGBColor(
                            255, 0, 0
                        )  # 翻译设置为红色
                else:
                    if translation:
                        p.add_run(translation).font.color.rgb = RGBColor(
                            255, 0, 0
                        )  # 翻译设置为红色
        doc.save(file_path)
    ...
    def translate_selected(self):
        ...
        if selected_item:
            ...
            if translated_text:
                ...
                original = self.tree.item(selected_item, "values")[0]
                self.data.loc[self.data["Original"] == original, "Translation"] = (
                    translated_text
                )
                logging.info(f"Translated text: {original_text} -> {translated_text}")
            else:
                messagebox.showerror(
                    "Translation Error", "Failed to translate the selected text."
                )
