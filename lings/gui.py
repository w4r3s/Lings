...
class TranslationApp:
    ...
    def update_categories(self, *args):
        ...
        categories = [
            ...
            if item["file"].startswith(selected_language)
        ]
        self.category_var.set("")
        ...
        for category in categories:
            menu.add_command(
                label=category, command=lambda value=category: self.set_category(value)
            )

    def set_category(self, category):
        self.category_var.set(category)
        category_file = next(
            (item["file"] for item in self.metadata if item["description"] == category),
            None,
        )
        if category_file:
    ...
    def create_widgets(self):
        tk.Label(self.root, text="Language (e.g., en_cn):").grid(
        ...
        tk.Label(self.root, text="Selected category path:").grid(
            row=2, column=0, padx=10, pady=10
        )
        tk.Label(self.root, textvariable=self.selected_category_path).grid(
            row=2, column=1, padx=10, pady=10
        )

        tk.Label(self.root, text="Selected file:").grid(
            row=3, column=0, padx=10, pady=10
        )
        tk.Label(self.root, textvariable=self.input_file_path).grid(
            row=3, column=1, padx=10, pady=10
        )

        tk.Button(self.root, text="Load Word File", command=self.load_file).grid(
            row=4, column=0, padx=10, pady=10
        )
        tk.Button(self.root, text="Add Translation", command=self.add_translation).grid(
            row=4, column=1, padx=10, pady=10
        )

        tk.Label(self.root, text="Strict punctuation:").grid(
            row=5, column=0, padx=10, pady=10
        )
        tk.Radiobutton(
            self.root, text="Yes", variable=self.strict_punctuation, value=True
        ).grid(row=5, column=1, padx=10, pady=10)
        tk.Radiobutton(
            self.root, text="No", variable=self.strict_punctuation, value=False
        ).grid(row=5, column=2, padx=10, pady=10)

        tk.Label(self.root, text="Ignore case:").grid(row=6, column=0, padx=10, pady=10)
        tk.Checkbutton(self.root, text="Yes", variable=self.ignore_case).grid(
            row=6, column=1, padx=10, pady=10
        )

        tk.Label(self.root, text="Partial match:").grid(
            row=7, column=0, padx=10, pady=10
        )
        tk.Checkbutton(self.root, text="Yes", variable=self.partial_match).grid(
        ...
        tk.Button(
            self.root, text="Translate Document", command=self.translate_document
        ).grid(row=9, column=0, columnspan=3, padx=10, pady=10)
        tk.Button(
            self.root, text="Manage Dictionaries", command=self.open_dictionary_manager
        ).grid(
            row=10, column=0, columnspan=3, padx=10, pady=10
        )  # 添加词库管理按钮
        tk.Button(
            self.root, text="Manage Plugins", command=self.open_plugin_manager
        ).grid(
            row=11, column=0, columnspan=3, padx=10, pady=10
        )  # 添加插件管理按钮
        tk.Button(self.root, text="Edit Document", command=self.open_text_editor).grid(
    ...
    def add_translation(self):
        word = simpledialog.askstring(
            "Input", "Enter the word or paragraph to translate:"
        )
        ...
        if word and translation:
            category_file = next(
                (
                    item["file"]
                    for item in self.metadata
                    if item["description"] == self.category_var.get()
                ),
                None,
            )
            if category_file:
                ...
                translator = Translator(
                    ...
                    category_path,
                    self.strict_punctuation.get(),
                    self.ignore_case.get(),
                    self.partial_match.get(),
                )
                translator.add_translation(word, translation)
                messagebox.showinfo(
                    "Info", f"Added translation: {word} -> {translation}"
                )
    ...
    def translate_document(self):
        ...
        if output_file:
            ...
            untranslated_segments = translator.translate_document(
                self.input_file, output_file, update_progress
            )
            if untranslated_segments:
                result = messagebox.askyesno(
                    "Edit Translations",
                    "Some segments were not translated. Would you like to edit them?",
                )
                if result:
                    open_editor(untranslated_segments, output_file)
            else:
                messagebox.showinfo(
                    "Info", f"Translated document saved as: {output_file}"
                )
